...
\begin{algorithmic}[1]
\Require{Replay buffer size $N$, Minibatch size $K$, discount factor $\gamma$, learning rates; $\lambda_{\alpha}, \lambda_{\pi}, \lambda_{Q}$, number of critics $C$, truncation amount $k$, number of quantiles $M$, target critic update rate $\beta$, update frequency $d$, Updates to Data ratio UTD, and neural network function approximator $Z$ and $\pi$}
\State Initialize replay buffer $\mathcal{D}$
