...
\begin{algorithmic}[1]
\Require{Clipping amount $\epsilon$, Minibatch size $K$, epoch $M$, discount factor $\gamma$, learning rates $\alpha_{\theta}$ and $\alpha_{\phi}$, number of actors $N$, environemt steps $T$, neural network function approximator $V$, neural network function approximator $\pi$ and value function coefficient $c_{1}$ and entropy coefficient $c_{2}$}
...
\begin{algorithmic}[1]
\Require{Replay buffer size $N$, Minibatch size $K$, discount factor $\gamma$, learning rates $\alpha$, $\alpha_{\pi}$ and $\alpha_{Q}$, target smoothing coefficient $\tau$, target policy smoothing noise $\sigma$, target policy noise clip $c$, target policy noise $\tilde{\sigma}$, target policy noise clip $c$, target policy noise $\tilde{\sigma}$, target policy noise clip $c$, update frequency $d$}
